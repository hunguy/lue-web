...
export default function App() {
      ...
      if (data.type === 'chapter_data') {
        ...
        setSentences(prev => {
          ...
          if (idx !== -1) {
            ...
            // New chapter crossed or not found
  ...
  useEffect(() => {
    ...
    const onLoadedData = () => {
      ...
      if (pendingSeekWordIdxRef.current !== null && s?.timing?.word_timings) {
        ...
        pendingSeekTimeRef.current = null; // Clear both
  ...
  }, [currentSentenceIndex, isUserScrolling]);
  ...
  const handleWordClick = (sentenceIdx: number, wIdx: number) => {
    const targetSentence = sentences[sentenceIdx];
    if (!targetSentence) return;
    
    // Force release of scroll lock so it can center immediately
    if (scrollTimeoutRef.current) clearTimeout(scrollTimeoutRef.current);
    setIsUserScrolling(false);
    
    // We must ALWAYS seek the backend so its sleep timer gets reset.
    // We store the word index because we might not have timing for this sentence yet!
    pendingSeekWordIdxRef.current = wIdx;
    
    // Wipe the audio_url immediately to prevent playing stale audio while backend generates new one
    setSentences(prev => {
      const newData = [...prev];
      if (newData[sentenceIdx]) {
        newData[sentenceIdx] = { ...newData[sentenceIdx], audio_url: undefined };
      }
      return newData;
    });
  ...
  if (!isReading) {
    return <Bookshelf onOpenBook={handleOpenBook} />;
  }

  const currentDuration = sentences[currentSentenceIndex]?.duration || 1;
  ...
  return (
    <div className="relative h-full w-full overflow-hidden font-sans select-none">
      ...
      <div className="relative z-10 flex items-center justify-between px-8 py-8">
        <div className="flex items-center gap-5">
          <motion.img 
            ...
            alt="Cover" 
            className="w-[60px] h-[60px] rounded-lg shadow-[0_8px_16px_rgba(0,0,0,0.5)]"
          />
          <div>
      ...
      <div 
        className="relative z-10 h-[60%] overflow-y-auto px-10 md:px-20 py-12 lyrics-container"
      ...
      >
        <div className="flex flex-col gap-12 pb-[30vh]">
          {sentences.map((line, sIdx) => {
            ...
            const isPast = sIdx < currentSentenceIndex;
            
            // Format words properly with timing data
            // The word_mapping helps map original words to TTS words
                  ...
                  {words.map((wordText: string, wIdx: number) => {
                    ...
                    return (
                      <motion.span
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          handleWordClick(sIdx, wIdx);
                        }}
                        className={`word text-4xl md:text-6xl font-bold cursor-pointer transition-all duration-400 ${
                          isWordActive 
      ...
      <div 
        className="absolute bottom-0 left-0 right-0 h-[100px] z-50 cursor-pointer"
        onClick={(e) => {
          e.stopPropagation();
          setShowControls(true);
        }}
      ...
      <AnimatePresence>
        {showControls && (
          <motion.div 
            ...
            className="absolute bottom-0 left-0 right-0 z-40 control-bar px-8 pt-16 pb-12"
          >
            <div className="max-w-4xl mx-auto">
              <div className="mb-8">
                <div className="relative w-full h-1 bg-white/10 rounded-full overflow-hidden shadow-inner">
                  <motion.div 
                    ...
                    style={{ width: `${Math.min(100, (currentTime / currentDuration) * 100)}%` }}
              ...
              <div className="flex items-center justify-center gap-10 mb-8">
                <button className="text-white/70 hover:text-white hover:scale-110 active:scale-95 transition-all" onClick={() => {
                   if (wsRef.current && wsRef.current.readyState === WebSocket.OPEN) {
                     wsRef.current.send(JSON.stringify({ command: 'prev_sentence' }));
                   }
                }}>
                ...
                <button 
                  ...
                  className="w-16 h-16 flex items-center justify-center bg-white text-black rounded-full hover:scale-110 active:scale-90 transition-all shadow-2xl"
                >
                  {isPlaying ? (
                ...
                <button className="text-white/70 hover:text-white hover:scale-110 active:scale-95 transition-all" onClick={() => {
                   if (wsRef.current && wsRef.current.readyState === WebSocket.OPEN) {
                     wsRef.current.send(JSON.stringify({ command: 'next_sentence' }));
                   }
                }}>
